...
void cy::algo_mover(void)
{
    ...
    std::vector<int> s{2,1,3,1,1,1,1,1,1};
    std::swap_ranges(v.begin(), v.end(), s.begin());
    cy::print("std::swap_ranges, s-> ", s);
}
...
void cy::algo_query_property(void)
{
    ...
    r = std::equal(n.begin(), n.end(), m.begin(), m.end());
