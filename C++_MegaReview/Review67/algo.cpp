...
{
    std::vector<int> v{2,1,3,99,88,99,0,-1,99};
...
void cy::algo_permutation_sort(void)
{
    ...
    /// psr
    std::partition(v.begin(), v.end(), [](const auto& e){ return (e&1) == 1; });
...
void cy::algo_mover(void)
{
    ...
    std::copy_backward(m.begin(), m.end(),
                       n.end());
    ...
    m = {"hello", "world"};
    n = {"zhang", "liang"};
    std::move_backward(m.begin(), m.end(),
                       n.end());
    cy::print("std::copy_backward, m-> ", m);
    cy::print("std::copy_backward, n-> ", n);
...
void cy::algo_query_value(void)
{
    ...
    std::vector<int> u{2,1,3,99,88,99,0,-1,99};
    rv = std::inner_product(v.begin(), v.end(),
                            u.begin(), .0);
    ...
    cy::oformat("std::adjacent_difference, diff-> ");
    std::adjacent_difference(v.begin(), v.end(),
                             std::ostream_iterator<int>(std::cout, " "));
    cy::newline();

    auto it = std::adjacent_find(v.begin(), v.end());
    cy::oformat("std::adjacent_found, it-> ");
    (it != std::end(v))
        ? std::cout << *it << std::endl
        : std::cout << "[not found]\n";
