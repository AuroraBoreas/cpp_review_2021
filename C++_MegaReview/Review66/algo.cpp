...
{
    std::vector<int> v{2,1,99,99,88,99,0,-1,99};
...
void cy::algo_mover(void)
{
    ...
    std::list<int> u{4,5};
    std::copy_backward(v.begin(), v.end(),
                       u.end());
    ...
    std::move_backward(v.begin(), v.end(), std::end(u));
...
void cy::algo_query_value(void)
{
    ...
    std::vector<int> u{2,1,99,99,88,99,0,-1,99};
...
void cy::algo_secret_runes(void)
{
    std::vector<int> v{2,1,99,99,88,99,0,-1,99};
    cy::print("original, v-> ", v);
    ...
    cy::oformat("std::secret_runes, -> nothing special;\n");
}
...
void cy::algo_lone_island(void)
{
    ...
    std::for_each(v.begin(), v.end(), [](const auto& e){ std::cout << e << cy::b; });
    cy::newline();
}
