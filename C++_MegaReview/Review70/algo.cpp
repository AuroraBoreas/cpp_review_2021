...
{
    std::vector<int> v{2,1,3,88,99,100,0,-1,99,1};
...
void cy::algo_permutation_sort(void)
{
    ...
    std::partial_sort(v.begin(), std::next(v.begin(), 4), v.end());
...
void cy::algo_query_value(void)
{
    ...
    std::vector<int> u{2,1,3,88,99,100,0,-1,99,1};
...
void cy::algo_query_property(void)
{
    ...
    rv = std::equal(n.begin(), n.end(), m.end());
    cy::oformat("std::equal, v-> ");
    std::cout << std::boolalpha << rv << std::endl;
